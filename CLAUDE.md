...
Vite dev server + static JSON data. No framework. Runtime merges design artifacts + per-store pricing feeds into absolute-positioned overlays on top of a background image.
...
├── package.json
├── src/
│   ├── app.js                      # registry → store/screen selects → fetches design + pricing
│   ├── renderer.js                 # merges design slots + pricing → DOM
│   └── styles.css
└── public/
    ├── assets/test-bg.png          # placeholder bg (to be replaced per-design from Figma)
    └── data/
...
- **Designs** — rare, generated by analyzing Figma JPGs. Each design is a self-contained artifact: background image + flat list of text slots with positions.
- **Pricing** — frequent, per-store. Comes from POS/pricing system. Independent of designs; overwriting a pricing file does not require regeneration.

At runtime the app resolves `storeId + screenId → designId` via `registry.json`, then fetches the relevant design file + pricing file and merges. Adding a 10th design = drop `design-j.json` in + wire into registry. No app changes.

### Schema

**Design** (`public/data/designs/design-*.json`):
```json
{
  "id": "design-a",
...
**Pricing** (`public/data/pricing/store-*.json`):
```json
{
  "storeId": "store-001",
  "items": {
...
Variant vocabulary is free-form strings (`meal`, `entree`, `meal-8ct`, `entree-12ct`, `base`, `with-toppings`, …) — no enum to maintain. The renderer resolves `pricing.items[itemId].variants[variantId][field]`; missing values render `—` per-slot, not per-item. A design referencing a variant the store doesn't carry shows `—` for that one slot only.
...
The inline `screens` map is a prototype shortcut. Production will introduce a middle `layouts` tier (~5–10 named layouts shared across ~500 stores, each with screen count + design assignments). Extracting this is a one-file change: stores reference `layoutId`, a new `layouts.json` holds the mappings. The renderer path doesn't change.

### What's next to wire

- **Figma MCP** — will produce real design files (image + slot positions) by analyzing Figma JPGs. Output shape must match the schema above. Placeholder coordinates in the current design files will be replaced once real designs are analyzed. Prefer Claude Code's built-in `figma-remote-mcp` tools.
